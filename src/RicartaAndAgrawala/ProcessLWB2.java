...
public class ProcessLWB2 {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        processLWB = new DatagramSocket(Message.LWB2_PORT);
        Message mssg;
        int timeStamp = 0;

        RAMutex raMutex = new RAMutex(Message.LWB2) {
            @Override
            ...
            void myWait() {
                try {
                    ...
                    if (message.getSrc() == Message.LWB1){
                        switch (message.getMode()) {
                            case Message.REQUEST:
                                handleMsg(message);
                                System.out.println("[DEBUG] <--LWB1 REQUEST");
                                break;
                            case Message.OKAY:
                                handleMsg(message);
                                System.out.println("[DEBUG] <--LWB1 OKAY");
                                break;
                            default:
        ...
        while (true){
            ...
            mssg = new Message(Message.TO_SCREEN, "Sóc el procés LWB2" , Message.LWB2);
            for (int i = 0; i < 10; i++) {
            ...
            mssg = new Message(Message.FINISHED, timeStamp, Message.LWB2);
            Message.sendMessage(processLWB, mssg, Message.B_PORT);
